...
const HotelCard = memo(({ hotel, isSelected, onSelect, t, isRtl, hotelImage, hotelDescription }: any) => (
  <Card className={cn(
    ...
    <div className="relative w-full aspect-video md:aspect-[16/10] shrink-0 overflow-hidden">
      ...
      {hotelImage && (
        <Image src={hotelImage.imageUrl} alt={hotelImage.description} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transition-transform duration-1000 group-hover:scale-110" data-ai-hint={hotelImage.imageHint} />
      )}
      ...
      <div className="pt-6 border-t border-gold/10 flex items-end justify-between">
        ...
        <div className="flex flex-col gap-2 items-end">
          <div className="flex gap-2">
            <Sheet>
              <SheetTrigger asChild><Button variant="outline" size="sm" className="border-gold/30 text-gold hover:bg-gold/5 h-9"><Info className="h-4 w-4" /></Button></SheetTrigger>
              <SheetContent side={isRtl ? "left" : "right"} className="glass-panel border-gold/20 w-full sm:max-w-xl p-0">
                <ScrollArea className="h-full">
                  <div className="relative h-64 w-full">
                    {hotelImage && <Image src={hotelImage.imageUrl} alt={hotelImage.description} fill className="object-cover" />}
                    ...
                    <div className="absolute bottom-6 start-6">
                      ...
                      <div className="flex items-center gap-2 text-gold font-bold text-xl"><span>{hotel.stars}</span><Star className="h-5 w-5 fill-gold" /></div>
                    </div>
                  </div>
                  <div className="p-8 space-y-10">
                    <div className="space-y-4">
                      ...
                      <SheetDescription className="text-base text-muted-foreground leading-relaxed italic border-s-2 border-gold/30 ps-4">{hotelDescription}</SheetDescription>
                    </div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-3 bg-white/5 border border-white/10 rounded-xl"><p className="text-[10px] text-muted-foreground uppercase flex items-center gap-1"><Users className="h-3 w-3" /> {t('hotels_triple')}</p><p className="text-sm font-bold text-gold">{hotel.pricingGrid.triple}</p></div>
                        <div className="p-3 bg-white/5 border border-white/10 rounded-xl"><p className="text-[10px] text-muted-foreground uppercase flex items-center gap-1"><Users className="h-3 w-3" /> {t('hotels_double')}</p><p className="text-sm font-bold text-gold">{hotel.pricingGrid.double}</p></div>
                        <div className="p-3 bg-white/5 border border-white/10 rounded-xl"><p className="text-[10px] text-muted-foreground uppercase flex items-center gap-1"><User className="h-3 w-3" /> {t('hotels_single')}</p><p className="text-sm font-bold text-gold">{hotel.pricingGrid.single}</p></div>
                        <div className="p-3 bg-white/5 border border-white/10 rounded-xl"><p className="text-[10px] text-muted-foreground uppercase flex items-center gap-1"><Baby className="h-3 w-3" /> {t('hotels_child_1')}</p><p className="text-sm font-bold text-gold">{hotel.pricingGrid.child1}</p></div>
                        <div className="p-3 bg-white/5 border border-white/10 rounded-xl"><p className="text-[10px] text-muted-foreground uppercase flex items-center gap-1"><Baby className="h-3 w-3" /> {t('hotels_child_2')}</p><p className="text-sm font-bold text-gold">{hotel.pricingGrid.child2}</p></div>
                        <div className="p-3 bg-white/5 border border-white/10 rounded-xl"><p className="text-[10px] text-muted-foreground uppercase flex items-center gap-1"><Baby className="h-3 w-3" /> {t('hotels_baby')}</p><p className="text-sm font-bold text-gold">{hotel.pricingGrid.baby}</p></div>
                      </div>
                    </div>
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">{hotel.amenities.map((amenity: string) => { const Icon = AMENITY_ICONS[amenity] || Info; return <div key={amenity} className="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-xl"><Icon className="h-4 w-4 text-gold" /><span className="text-sm font-medium">{amenity}</span></div>; })}</div>
                    </div>
                    <div className="pt-8 border-t border-gold/10"><Button onClick={() => onSelect(hotel.id)} className={cn("w-full h-14 text-lg font-bold", isSelected ? "bg-emerald-500 hover:bg-emerald-600 text-white" : "bg-gold hover:bg-gold/80 text-gold-foreground")}>{isSelected ? <><Check className="h-5 w-5 me-2" /> Sélectionné</> : t('hotels_drawer_cta')}</Button></div>
                  </div>
                </ScrollArea>
              </SheetContent>
            </Sheet>
...
export const Hotels = memo(({ selectedId, onSelect }: any) => {
  ...
  return (
    <div className="space-y-12">
      <div className="p-4 bg-primary/5 border-s-4 border-primary rounded-e-lg text-sm text-foreground/80 leading-relaxed max-w-2xl mx-auto"><strong>{t('hotels_note').split(':')[0]} :</strong>{t('hotels_note').split(':')[1]}</div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
        {TRIP_CONFIG.hotels.map((hotel) => (
          <HotelCard key={hotel.id} hotel={hotel} isSelected={selectedId === hotel.id} onSelect={onSelect} t={t} isRtl={isRtl} hotelImage={PlaceHolderImages.find(img => img.id === hotel.imageId)} hotelDescription={language === 'ar' ? `فندق ${hotel.stars} نجوم في شرم الشيخ.` : `Hôtel ${hotel.stars} étoiles à Sharm El Sheikh.`} />
        ))}
      </div>
      <Card className="bg-gold/5 border-dashed border-gold/40 max-w-2xl mx-auto"><CardContent className="p-6 flex items-start gap-4"><MapPin className="h-6 w-6 text-gold flex-shrink-0" /><div className="text-sm"><h5 className="font-bold text-gold mb-1 uppercase tracking-wide">{t('hotels_cairo_title')}</h5><p className="text-muted-foreground leading-relaxed">{t('hotels_cairo_desc')}</p></div></CardContent></Card>
    </div>
  );
});
